...
def c_space_fig():
    ...
    #pltr.scatter(ax, C, c=[[.6*g for _ in range(3)] for g in interp],edgecolor='face')
    pltr.scatter(ax, C, s=20, c=[str(.6*g) for g in interp],edgecolor='face')
    ...
    for p in range(num_C):
        ...
        for i in range(len(VA)):
            for s in [1]:#[-1,1]:
                pltr.plot(ax, s*VA[i][:N, (np.fabs(VA[i][:N,:]) < 3).all(axis=0)], str(.6*interp[p]))
